...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        description="知微：社交媒体舆情监测（BERT 情感 + BERTopic 主题 + 趋势预警；支持 B 站评论口碑分析）",
        lifespan=lifespan,
    )
    app.add_middleware(
        CORSMiddleware,
    ...
    async def unauthorized_exception_handler(_request, exc: UnauthorizedError):
        return err(exc.code, exc.message, status=401)
    ...
    app.include_router(collect_router, prefix="/api/v1", dependencies=protected)
    app.include_router(analysis_router, prefix="/api/v1", dependencies=protected)
    app.include_router(alerts_router, prefix="/api/v1", dependencies=protected)
