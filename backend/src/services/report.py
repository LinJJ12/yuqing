...
def _register_cjk_font() -> str:
    ...
    if "ZhiweiCJK" in pdfmetrics.getRegisteredFontNames():
        return "ZhiweiCJK"
    ...
    try:
        if font_path.lower().endswith(".ttc"):
            pdfmetrics.registerFont(TTFont("ZhiweiCJK", font_path, subfontIndex=0))
        else:
            pdfmetrics.registerFont(TTFont("ZhiweiCJK", font_path))
        return "ZhiweiCJK"
    except Exception:
        try:
            pdfmetrics.registerFont(TTFont("ZhiweiCJK", font_path, subfontIndex=1))
            return "ZhiweiCJK"
        except Exception:
            return "Helvetica"
...
def build_pdf_bytes(summary: dict[str, Any] | None = None) -> bytes:
    ...
    story: list[Any] = []
    story.append(Paragraph(_xml(data.get("generated_for") or "知微报告"), title_style))
    story.append(
        Paragraph(
...
def generate_ai_summary(summary: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    prompt = (
        "你是知微分析助手。根据以下统计写一段 150～250 字中文摘要，"
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "你擅长简洁清晰的知微舆情简报。"},
