...
> 版本：0.1 · 状态：现行产品说明  
> 关联：[定位与演进](./positioning.md) · [架构图](./diagrams.md) · [真实采集](./real-data-collection.md)
...
| 角色 | 要解决什么 | 典型用法 |
|------|------------|----------|
| **UP 主 / 内容团队** | 单视频或一批视频「观众怎么说」 | 贴 BV → 采集评论 → 看情感与词云 → 出单视频口碑报告 |
| **运营 / 品宣** | 话题或内容是否翻车、负面是否突增 | 监测入库 → 预警中心 → PDF/CSV 汇总给协作方 |
| **学生 / 大创答辩** | 可演示的完整闭环（采集→分析→报告） | 样例数据或真实 BV；设置页检查模型就绪 |
| **研究者 / 二次开发** | 可替换模型栈的本地分析底座 | FastAPI + SQLite；改 `services/` 与 `.env` |
...
| 模块 | 用户可见能力 | 关键能力 |
|------|--------------|----------|
| **舆情监测** | BV 评论采集（主）、关键词采集（次）、JSON/CSV 导入、入库浏览与噪声清理 | 内嵌 B 站采集、质量门禁（标题/空评/刷评）、文件 ingest |
| **总览** | 帖子量、话题与情感快照 | dashboard overview |
| **情感分析** | 预览、批量跑批、统计；难例人工/LLM 改判 | 中文 BERT 三分类 + uncertain；低置信可复核 |
| **热点话题** | 词云、关键词、主题聚类 | jieba / TF-IDF + BERTopic（Ollama 向量） |
| **预警中心** | 负面/敏感词命中、热度突增列表；敏感词可配置 | 规则引擎 + 趋势日聚合 |
| **趋势** | 滑动平均；可选 Prophet | `forecast` |
| **分析报告** | 全局汇总；单视频口碑；CSV/PDF 导出；可选 AI 摘要 | `report` / `video_report` |
| **智能助手** | 基于库内数据的问答与简报（可限定 BV） | OpenAI 兼容或 Ollama Chat |
| **系统设置** | 情感模型 / Ollama / 云端 LLM 就绪检查；设备信息；敏感词 | readiness |
...
```text
① 监测页贴 BV（或导入文件）→ 入库
② 情感页跑批（或采集后自动入队）→ 主题页出词云/聚类
③ 总览 / 预警看异常 → 报告页出全局或单视频口碑
④ （可选）助手页问答 / 生成简报 → PDF/CSV 交付
```
...
- [ ] 仅用真实 BV，可不依赖校园样例完成「采集→情感→主题→单视频报告」
- [ ] 设置页能区分情感缓存 / **B 站 Cookie** / Ollama / 云端 LLM 就绪状态，并有「演示前 3 步」清单
- [ ] 空库总览给出三步首跑引导（监测 → 情感 → 报告），而非空图表
- [ ] 情感未跑完时单视频报告标明「未完成」，不假装口碑已定论
- [ ] 预警敏感词可改且立即影响后续判定；placeholder 为口碑向示例
- [ ] 报告可导出 PDF 与 CSV
